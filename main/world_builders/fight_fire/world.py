...
class World:
    def __init__(world, *, grid_width, grid_height, fire_locations=None, water_locations=None, visualize=False, debug=False, random_seed=None, corridor_mode=False, fast_as_possible=None):
        ...
        world.user_given_water_locations = water_locations or []
        world.reset()
    ...
    def reset(world):
        ...
        if len(world.user_given_fire_locations ) > 0:
            world.state.grid.fire  = torch.zeros(world.state.grid.fire.shape ) != 0
            for x,y in world.user_given_fire_locations:
                world.state.grid.fire[x,y] = True
        if len(world.user_given_water_locations) > 0:
            world.state.grid.water = torch.zeros(world.state.grid.water.shape) != 0
            for x,y in world.user_given_water_locations:
                world.state.grid.water[x,y] = True
